...
def creat_data(datafile, cellfile):
    ...
    for smile in compound_iso_smiles:
        ...
        morgan_fingerprints[smile] = fp.tolist()  # 转换为列表便于JSON序列化

    # 确保目录存在并保存指纹文件
    os.makedirs('./data/processed/', exist_ok=True)
    with open('./data/processed/morgan_fingerprints.json', 'w') as f:
        json.dump(morgan_fingerprints, f)
    print(f"摩根指纹生成完成！共处理 {len(morgan_fingerprints)} 个分子")
    print(f"指纹文件保存至: ./data/processed/morgan_fingerprints.json")
    # === 指纹生成结束 ===
    ...
    if ((not os.path.isfile(processed_data_file_train))):
        ...
        TestbedDataset(root='data', dataset=datafile + '_drug1',
                      ...
                      smile_graph=smile_graph, fingerprint_dict=morgan_fingerprints)  # 新增参数
